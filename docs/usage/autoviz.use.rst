...
**Note**: While this guide focuses on NetCDF input data, EViz also supports GRIB, HDF, Zarr, and CSV files. EViz identifies data sources by file extension and assumes NetCDF4 format if no extension is provided. Remote data can be accessed via OPeNDAP URLs.
...
2. On NASA's DISCOVER supercomputing system you can try this example by copying the file
...
maintain files in a separate config directory and use those when running autoViz. In that case you can specify
...
Also, note that on NASA's DISCOVER system there are sample config files that you can copy to your work space. For example:
