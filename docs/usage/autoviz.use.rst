...
For this case we run **autoViz** as follows (asumming EVIZ_CONFIG_PATH=config/examples):
...
.. note::
   The `geos` and `ccm` options are actually gridded sources. However, in `Eviz` we implemented
   classes with particular methods and requirements to handle cases that correspond to the GEOS and 
   CCM models respectively. Thus, you can use `-s gridded` to process  GEOS and CCM output but it will
...
For details look in the corresponding YAML files in config/examples/geos and config/ccm and feel free to change the 
settings.
...
WRF configurations can be found in the config/examples/wrf folder. Feel free to modify the filename (and location) entries.
...
This will produce simple LatLon and zonal mean plots for some fields specified in the config/examples/wrf.yaml file.

Visualize with a predefined LIS model
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

LIS configurations can be found in the config/examples/lis folder. To generate the plots run:
...
This will produce simple 2D (LatLon) plots for the fields specified in the config/examples/lis.yaml file.
