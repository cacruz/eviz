...
class Figure(mfigure.Figure):
    ...
    def _set_compare_diff_subplots(self):
        ...
        try:
            ...
            self.logger.warning(
                f"Error setting subplot layout: {str(e)}, using default"
            )
            self._subplots = (1, 1)
    ...
    def create_subplot_grid(self) -> "Figure":
        ...
        if self._frame_params[self._rindex][2] and self._frame_params[self._rindex][3]:
            figsize = (
                self._frame_params[self._rindex][2],
                self._frame_params[self._rindex][3],
            )
    ...
    def _create_subplots_crs(self) -> "Figure":
        ...
        if hasattr(self, "field_name") and self.field_name:
            if (
                ...
                and self.field_name in self.config_manager.spec_data
            ):
                # Check for projection at the top level of the field spec
                if "projection" in self.config_manager.spec_data[self.field_name]:
                    projection_name = self.config_manager.spec_data[self.field_name][
                        "projection"
                    ]
                    map_projection = self._get_projection(projection_name)
                    self._logger.debug(
                        f"Using projection '{projection_name}' for field {self.field_name}"
                    )
                # Also check in the plot-type specific section
                elif "projection" in self.config_manager.spec_data[self.field_name].get(
                    f"{self.plot_type[:2]}plot", {}
                ):
                    projection_name = self.config_manager.spec_data[self.field_name][
                        f"{self.plot_type[:2]}plot"
                    ]["projection"]
    ...
    def have_multiple_axes(self):
        return self.axes is not None and (
            self.axes.numRows > 1 or self.axes.numCols > 1
        )
    ...
    def init_ax_opts(self, field_name) -> Dict[str, Any]:
        ...
        spec = self.config_manager.spec_data.get(field_name, {}).get(
            f"{plot_type}plot", {}
        )

        existing_rc_params = {}
        if hasattr(self, "_ax_opts") and "rc_params" in self._ax_opts:
            existing_rc_params = self._ax_opts.get(
                "rc_params", {}
            ).copy()  # Make a copy
        ...
        filtered_rc_params = {
            k: v for k, v in rc_params_from_yaml.items() if k in rc_keys
        }
    ...
    def update_ax_opts(self, field_name, ax, pid, level=None) -> Dict[str, Any]:
        ...
        if self._subplots == (3, 1) and geom[1:] == (0, 1, 1, 1):
            ...
            self._ax_opts["clevs"] = self.config_manager.yaml_parser.spec_data[
                field_name
            ].get(diff_opt, None)
        else:
            self._set_clevs(
    ...
    def _set_clevs(self, field_name, ptype, ctype):
        ...
        if "contours" in self.config_manager.spec_data[field_name][ptype]:
            self._ax_opts["clevs"] = self.config_manager.spec_data[field_name][ptype][
                "contours"
            ]
            return
        ...
        if isinstance(ctype, int):
            if ctype in self.config_manager.spec_data[field_name][ptype]["levels"]:
                self._ax_opts["clevs"] = self.config_manager.spec_data[field_name][
                    ptype
                ]["levels"][ctype]
    ...
    def _calculate_subplot_spacing(self):
        ...
        return {"wspace": base_wspace, "hspace": base_hspace}
    ...
    def _get_plot_config(cls, config_manager, field_name, plot_type):
        ...
        if config_manager.spec_data and spec_field_name in config_manager.spec_data:
            ...
            plot_config["projection"] = plot_config["projection"] or field_spec.get(
                "projection"
            )
    ...
    def _determine_subplot_layout(
        cls, config_manager, field_name, plot_type, nrows, ncols
    ):
        ...
        if config_manager.compare and field_name:
            try:
                use_overlay = config_manager.should_overlay_plots(
                    field_name, plot_type[:2]
                )
        ...
        if config_manager.compare and not config_manager.compare_diff:
            ...
            if hasattr(config_manager, "input_config") and hasattr(
                config_manager.input_config, "_comp_panels"
            ):
