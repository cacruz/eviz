...
class AltairMetricPlotter(XYPlotter):
    ...
    def compute_correlation_map(
        self, data1: xr.DataArray, data2: xr.DataArray
    ) -> xr.DataArray:
        ...
        if len(dims1) == 3:
            ...
            self.logger.debug(
                f"Computing correlation across {time_len} time points for a "
            ...
            for i in range(y_len):
                for j in range(x_len):
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Error computing correlation at point " f"({i},{j}): {e}"
                        )
                        corr_data[i, j] = np.nan
            ...
            corr_data.attrs["description"] = (
                f"Correlation between {data1.name} and {data2.name} across time"
            )
        ...
        # Handle 2D data
            ...
            try:
                ...
                corr_data.attrs["long_name"] = (
                    "Pearson Correlation Coefficient (Spatial)"
                )
                corr_data.attrs["units"] = "dimensionless"
                corr_data.attrs["description"] = (
                    f"Spatial correlation between {data1.name} and {data2.name}"
                )
    ...
    def plot(self, config: "ConfigManager", data_to_plot: tuple) -> alt.Chart:
        ...
        try:
            ...
            self.logger.debug(
                f"Data extent: lon [{x_min}, {x_max}], lat [{y_min}, {y_max}]"
            )
            ...
            if "extent" in config.ax_opts:
                ...
                self.logger.debug(
                    f"Using provided extent: lon [{x_min}, {x_max}], lat [{y_min}, {y_max}]"
                )
            else:
                # For continental US, we can set a fixed extent if the data is in that region
                is_us_region = (
                    x_min >= -130 and x_max <= -65 and y_min >= 23 and y_max <= 51
                )
                ...
                if is_us_region:
                    ...
                    y_min, y_max = 23, 51
                    self.logger.debug(
                        f"Setting extent to continental US: "
            ...
            if irregular_grid:
                ...
                try:
                    ...
                    data2d = new_data
                    self.logger.debug(
                        "Successfully interpolated irregular grid to regular grid"
                    )
                except Exception as e:
            ...
            base = alt.Chart(df).encode(
                ...
                ),  # Set explicit domain for x
                y=alt.Y(
                    f"{y_dim}:Q",
                    title="Latitude",
                    scale=alt.Scale(domain=[y_min, y_max]),
                ),  # Set explicit domain for y
            )
            ...
            try:
                ...
                state_borders = (
                    alt.Chart(states)
                    .mark_geoshape(filled=False, stroke="black", strokeWidth=0.5)
                    .project(
