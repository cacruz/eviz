...
class AltairScatterPlotter(ScatterPlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        try:
            x_label = ax_opts.get(
                "xlabel", x_data.name if hasattr(x_data, "name") else "X"
            )
            y_label = ax_opts.get(
                "ylabel", y_data.name if hasattr(y_data, "name") else "Y"
            )
            ...
            if z_data is not None:
                chart = (
                    alt.Chart(df)
                    .mark_circle(size=marker_size)
                    .encode(
                        x=alt.X("x:Q", title=x_label),
                        y=alt.Y("y:Q", title=y_label),
                        color=alt.Color(
                            "z:Q", scale=alt.Scale(scheme=vega_scheme), title=units
                        ),
                    )
                )
            else:
            ...
            chart = chart.properties(width=800, height=500, title=title).interactive()
            ...
            chart = (
                ...
                .encode(x="x:Q", y="y:Q")
                .properties(
                    width=800, height=500, title=f"{title} (fallback visualization)"
                )
            )
    ...
    def _convert_to_dataframe(self, x_data, y_data, z_data=None):
        ...
        try:
            ...
            if z_data is not None:
                z_values = (
                    z_data.values if hasattr(z_data, "values") else np.array(z_data)
                )
                ...
                df = pd.DataFrame({"x": x_values[:min_len], "y": y_values[:min_len]})
            ...
            if z_data is not None:
                return pd.DataFrame({"x": [0, 1, 2], "y": [0, 1, 2], "z": [0, 1, 2]})
            else:
