...
class AltairXYPlotter(XYPlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        if not outliers.empty:
            self.logger.debug(
                f"Found {len(outliers)} outliers outside range [{lower_bound}, {upper_bound}]"
            )

        if ax_opts.get("clevs") is not None:
            color_domain = ax_opts["clevs"]
            self.logger.debug(
                f"Using specified contour levels for color domain: {color_domain}"
            )
        else:
            # Use extent
            vmin = df["value"].min()
        ...
        try:
            ...
            chart = (
                ...
                .encode(
                    x=alt.X(
                        ...
                        title=x_name,
                        scale=alt.Scale(
                            domain=[df["x"].min(), df["x"].max()], nice=False
                        ),
                    ),
                    y=alt.Y(
                        "y:Q",
                        ...
                        sort=y_sort,
                        scale=alt.Scale(
                            domain=[df["y"].min(), df["y"].max()], nice=False
                        ),
                    ),
                    color=alt.Color(
                        "value:Q",
            ...
            chart = (
                alt.Chart(df)
                .mark_point()
                .encode(x="x:Q", y="y:Q", color="value:Q")
                .properties(
    ...
    def _convert_to_dataframe(self, data2d, x, y):
        ...
        try:
            ...
            if isinstance(data2d, pd.DataFrame):
                if "x" in data2d.columns and "y" in data2d.columns:
                    if "value" not in data2d.columns:
                        ...
                        value_cols = [
                            col for col in data2d.columns if col not in ["x", "y"]
                        ]
                        if value_cols:
            ...
            self.logger.debug(
                f"Data values range: {np.nanmin(data_values)} to {np.nanmax(data_values)}"
            )
            ...
            return pd.DataFrame({"x": [0, 1], "y": [0, 1], "value": [0, 1]})
