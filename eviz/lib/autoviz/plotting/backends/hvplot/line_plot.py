...
class HvplotLinePlotter(BasePlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        ----------
            ...
            data_to_plot
                Tuple containing (data, x, y, field_name, plot_type, findex, fig)
        ...
        self.logger.debug(
            f"Data shape: {data.shape if hasattr(data, 'shape') else 'scalar'}"
        )
        ...
        if (
            "lineplot" in config.spec_data[field_name]
            and "fill_value" in config.spec_data[field_name]["lineplot"]
        ):
        ...
        try:
            if isinstance(data, pd.DataFrame):
                ...
                if x is not None and y is not None:
                    ...
                    x_dim = data.dims[0]
                    df = pd.DataFrame({"x": data[x_dim].values, "y": data.values})
                else:
                    # Simple arrays
            ...
            if ax_opts.get("markers", False):
                scatter = df.hvplot.scatter(x=x_col, y=y_col, color="red", size=50)
                plot = plot * scatter
