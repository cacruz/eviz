    ...
    def plot(self, config, data_to_plot):
        ...
        try:
            ...
            if ax_opts.get("add_regression", False):
                try:
                    ...
                    line_plot = line_df.hvplot.line(
                        x="x", y="y", color="red", line_width=2
                    )

                    r_squared = r_value**2
                    text = hv.Text(
                        df["x"].min() + 0.1 * (df["x"].max() - df["x"].min()),
            ...
            try:
                ...
                x_values = (
                    x_data.values if hasattr(x_data, "values") else np.array(x_data)
                )
                y_values = (
                    y_data.values if hasattr(y_data, "values") else np.array(y_data)
                )
                ...
                if z_data is not None:
                    ...
                    scatter = hv.Scatter((x_values, y_values), kdims=[x_label, y_label])
