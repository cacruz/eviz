    ...
    def plot(self, config, data_to_plot):
        ...
        try:
            if hasattr(data2d, "hvplot"):
                ...
                if window_size > 0:
                    self.logger.debug(
                        f"Applying rolling mean with window size {window_size}"
                    )
                ...
                if (
                    ...
                    if (
                        "add_trend" in config.spec_data[field_name]["xtplot"]
                        and config.spec_data[field_name]["xtplot"]["add_trend"]
                    ):
                ...
                if (
                    ...
                    if (
                        ...
                        if isinstance(
                            df["time"].iloc[0], (pd.Timestamp, np.datetime64)
                        ):
                            x_numeric = (
            ...
            try:
                ...
                curve = hv.Curve(df, "time", "value")
                plot = curve.opts(title=title, width=800, height=400)
    ...
    def _convert_to_dataframe(self, data2d, time_coords):
        ...
        try:
            ...
            if len(time_coords) != len(values):
                self.logger.warning(
                    f"Time coordinates length ({len(time_coords)}) "
            ...
            if len(time_coords) > 0 and (
                ...
                try:
                    ...
                    time_coords = pd.date_range(
                        start="2000-01-01", periods=len(values), freq="D"
                    )

            df = pd.DataFrame({"time": time_coords, "value": values})
