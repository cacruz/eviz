...
class MatplotlibCSVHistPlotter(MatplotlibBasePlotter):
    ...
    def _plot_hist_data(self, config, data, field_name, plot_options, plot_params):
        ...
        ----------
            ...
            plot_params
                Dictionary of plot parameters (x for categorical data)
        """
        ax = self.ax
        ...
        with mpl.rc_context(rc=self.ax_opts.get("rc_params", {})):
            ...
            if plot_options.get("show_std", False):
                ...
                if orientation == "vertical":
                    ...
                    ax.axvline(
                        mean_val + std_val, color="orange", linestyle=":", linewidth=1.5
                    )
                else:
                    ax.axhline(
                        mean_val - std_val,
            ...
            if (
                plot_options.get("show_mean", False)
                or plot_options.get("show_median", False)
                or plot_options.get("show_std", False)
            ):
            ...
            self.logger.info(
                f"Created histogram for {field_name} with {len(bin_edges)-1} bins"
            )
