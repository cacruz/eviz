...
class MatplotlibMetricPlotter(MatplotlibBasePlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        self._plot_correlation_data(
            config, data2d, x, y, field_name, fig, findex, method_name
        )
        return fig

    def _plot_correlation_data(
        self, config, data2d, x, y, field_name, fig, findex, method_name="Pearson"
    ):
        ax = self.ax
    ...
    def _compute_correlation_map(self, corr_method, data1, data2):
        ...
        if data1.shape != data2.shape:
            self.logger.warning(
                f"Data shapes don't match: {data1.shape} vs {data2.shape}, "
                ...
                for j in range(x_len):
                    ...
                    try:
                        if corr_method == "rmse":
                            ...
                            # Normalize the data
                            ts1_norm = (ts1[mask] - np.mean(ts1[mask])) / (
                                np.std(ts1[mask]) or 1
                            )
                            ts2_norm = (ts2[mask] - np.mean(ts2[mask])) / (
                                np.std(ts2[mask]) or 1
                            )

                            cross_corr = correlate(
                                ts1_norm, ts2_norm, mode="valid"
                            ) / len(ts1_norm)
        ...
        # Handle 2D data
            ...
            self.logger.warning(
                f"Input data is 2D, computing spatial {corr_method} correlation instead "
            ...
            try:
                if corr_method == "pearson":
                    ...
                    # Normalize the data
                    flat1_norm = (flat1[mask] - np.mean(flat1[mask])) / (
                        np.std(flat1[mask]) or 1
                    )
                    flat2_norm = (flat2[mask] - np.mean(flat2[mask])) / (
                        np.std(flat2[mask]) or 1
                    )

                    cross_corr = correlate(flat1_norm, flat2_norm, mode="valid") / len(
                        flat1_norm
                    )
                ...
                corr_data.attrs["long_name"] = (
                    f"{method_name} Correlation Coefficient (Spatial)"
                )
                corr_data.attrs["units"] = "dimensionless"
                corr_data.attrs["description"] = (
                    f"Spatial {method_name} correlation between {data1.name} and {data2.name}"
                )
                corr_data.attrs["correlation_method"] = corr_method
                ...
                self.logger.error(
                    f"Error computing spatial {corr_method} correlation: {e}"
                )
                return None
        else:
