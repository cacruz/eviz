...
def simple_xy_plot(
    ...
    source_name = (
        config.input_config.source_names[0]
        if config.input_config.source_names
        else "gridded"
    )
    ...
    if source_name in ["lis", "wrf"]:
        ...
        cf = ax.contourf(
            dim1.values, dim2.values, data2d.values, cmap=config.output_config.colormap
        )
        co = ax.contour(
            dim1.values, dim2.values, data2d.values, levels, linewidths=(1,), colors="k"
        )
    else:
    ...
    def plot(
        ...
        if plot_type == "xy":
            ...
            if lon_coord is not None and lat_coord is not None:
                ...
                self.logger.error(
                    f"Could not find longitude/latitude coordinates for {field_name}"
                )
            ...
            if vert_coord is not None and other_coord is not None:
                ...
                self.logger.error(
                    f"Could not find appropriate coordinates for YZ plot of {field_name}"
                )

        else:
            self.logger.warning(
                f"Simple plot type '{plot_type}' not supported. Defaulting to XY plot."
            )
            # Default to xy plot
