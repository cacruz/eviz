...
class MatplotlibTXPlotter(MatplotlibBasePlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        with mpl.rc_context(rc=self.ax_opts.get("rc_params", {})):
            ...
            cbar = fig.colorbar(
                cfilled, orientation="horizontal", pad=0.1, aspect=70, extendrect=True
            )
    ...
    def _process_data_for_hovmoller(self, data2d, vtimes, lons):
                ...
                if data2d.shape[0] == len(vtimes):
                    ...
                    # data shape is (time, lat, lon), we need to average over lat
                        ...
                        if time_dim_idx is not None and lon_dim_idx is not None:
                            ...
                            data2d_reduced = data2d.reshape(data2d.shape[0], -1).mean(
                                axis=1
                            )
        ...
        if hasattr(data2d_reduced, "shape") and len(data2d_reduced.shape) >= 2:
            if data2d_reduced.shape[0] != len(vtimes) or data2d_reduced.shape[1] != len(
                lons
            ):
    ...
    def _setup_map_panel(ax):
        ...
        # Set latitude ticks
        ax.set_yticks([40, 60])
        ax.set_yticklabels(["40\N{DEGREE SIGN}N", "60\N{DEGREE SIGN}N"])
