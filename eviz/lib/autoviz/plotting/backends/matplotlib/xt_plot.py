...
class MatplotlibXTPlotter(MatplotlibBasePlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        self._plot_xt_data(
            config, fig, data2d, field_name, findex, global_vmin, global_vmax
        )
        ...
        if config.overlay:
            ...
            all_plotted = (
                getattr(config, "current_dataset_index", 0)
                == getattr(config, "total_datasets", 1) - 1
            )
            if all_plotted or self.ax_opts.get("force_legend", False):
                legend = self.ax.legend(
                    loc="best", fontsize=self._legend_font_size(fig.subplots)
                )
    ...
    def _plot_xt_data(
        ...
        with mpl.rc_context(rc=ax_opts.get("rc_params", {})):
            ...
            try:
                ...
                if (
                    hasattr(time_coords[0], "_cftime")
                    or str(type(time_coords[0])).find("cftime") >= 0
                ):
                    try:
                        ...
                        self.logger.warning(
                            f"Error converting cftime to pandas datetime: {e}"
                        )
                        time_coords = np.arange(len(data2d))
            ...
            if (
                ...
                and "xtplot" in config.spec_data[field_name]
            ):
                if "mean_type" in config.spec_data[field_name]["xtplot"]:
                    if config.spec_data[field_name]["xtplot"]["mean_type"] == "rolling":
                        if "window_size" in config.spec_data[field_name]["xtplot"]:
                            window_size = config.spec_data[field_name]["xtplot"][
                                "window_size"
                            ]
            ...
            if config.overlay:
                ...
                if hasattr(config, "a_list") and hasattr(config, "b_list"):
                    ...
                    label = config.get_file_exp_name(findex) or config.get_file_exp_id(
                        findex
                    )
                    if not label:
            ...
            if window_size > 0 and len(data2d) > 2 * window_size:
                ...
                if line_color or label:
                    ax.plot(
                        time_coords[window_size:end_idx],
                        data2d[window_size:end_idx],
                        color=line_color,
                    ...
                    ax.plot(
                        time_coords[window_size:end_idx], data2d[window_size:end_idx]
                    )
            else:
                if line_color or label:
                    ax.plot(
                ...
                if "add_trend" in config.spec_data[field_name]["xtplot"]:
                    ...
                    if config.spec_data[field_name]["xtplot"]["add_trend"]:
                        try:
                            ...
                            if (
                                "trend_polyfit"
                                in config.spec_data[field_name]["xtplot"]
                            ):
                                degree = config.spec_data[field_name]["xtplot"][
                                    "trend_polyfit"
                                ]
                            else:
                                errors = []
            ...
            try:
                ...
                if (
                    global_vmin is not None
                    and global_vmax is not None
                    and config.overlay
                ):
                    plot_dmin, plot_dmax = global_vmin, global_vmax
                    self.logger.debug(
                        f"Using global y-axis range for overlay: [{plot_dmin:.6f}, {plot_dmax:.6f}]"
                    )
                else:
