...
class MatplotlibYZPlotter(MatplotlibBasePlotter):
    ...
    def plot(self, config, data_to_plot):
        ...
        if axes_shape == (3, 1):
            ...
            if isinstance(ax_temp, list):
                self.ax = ax_temp[
                    config.axindex
                ]  # Use the correct axis based on axindex
            else:
                self.ax = ax_temp
        else:
            self.ax = ax_temp[0]
            ...
            if ax_opts["profile_dim"]:
                ...
                if config.overlay:
                    ...
                    if hasattr(config, "a_list") and hasattr(config, "b_list"):
                        ...
                        label = config.get_file_exp_name(
                            findex
                        ) or config.get_file_exp_id(findex)
                        if not label:
                ...
                if config.overlay:
                    all_plotted = False
                    if hasattr(config, "current_dataset_index") and hasattr(
                        config, "total_datasets"
                    ):
                        all_plotted = (
                            config.current_dataset_index == config.total_datasets - 1
                        )

                    if all_plotted or ax_opts.get("force_legend", False):
                        legend = ax.legend(
                            loc="best", fontsize=pu.legend_font_size(fig.subplots)
                        )
    ...
    def _set_ax_ranges(self, config, field_name, fig, ax, y, units):
        ...
        if units == "Pa":
            ...
            if y.min() <= 20.0:
                y_ranges = np.append(
                    y_ranges, np.array([7, 5, 3, 2, 1, 0.7, 0.5, 0.3, 0.2, 0.1]) * 100
                )
            if y_ranges[-1] != y.min():
            ...
            if y.min() <= 0.2:
                y_ranges = np.append(
                    y_ranges, np.array([7, 5, 3, 2, 1, 0.7, 0.5, 0.3, 0.2, 0.1])
                )
            if y_ranges[-1] != y.min():
                y_ranges = np.append(y_ranges, y.min())
