...
class PlotManager:
    ...
    def plot(self):
        ...
        elif has_categorical and not (
            self.config_manager.compare
            or self.config_manager.compare_diff
            or self.config_manager.overlay
            or self.config_manager.correlation
        ):
        ...
        if self.config_manager.print_to_file:
            ...
            for i in range(len(self.config_manager.map_params)):
                if self.config_manager.compare or self.config_manager.compare_diff:
                    entry = u.get_nested_key_value(
                        self.config_manager.map_params[i], ["outputs", "output_dir"]
                    )
                    if entry:
    ...
    def _get_filename_for_index(self, file_index: int) -> str:
        ...
        """
        try:
            if file_index is not None and file_index < len(
                self.config_manager.app_data.inputs
            ):
    ...
    def _get_data_extent(self, data_array):
        ...
        try:
            ...
            if (
                lon_coord_name in data_array.coords
                and lat_coord_name in data_array.coords
            ):
    ...
    def _prepare_field_to_plot(
        ...
        self.logger.debug(
            f"Preparing data for: {field_name}, plot_type:{plot_type}, time_level:{time_level}, level:{level}"
        )
        if "xy" in plot_type or "polar" in plot_type:
            data2d = self.data_extractor._extract_xy_data(
                data_array, time_level, level=level
            )
            ...
            data2d = self.data_extractor._extract_tx_data(
                data_array, time_level, level=level
            )
            ...
            data2d = self.data_extractor._extract_line_data(
                data_array, time_level, level=level
            )
        elif "box" in plot_type:
            data2d = self.data_extractor._extract_box_data(
                data_array,
                time_level,
                exp_id=self.config_manager.get_file_exp_id(file_index),
            )
        elif "corr" in plot_type:
            data2d = self.data_extractor._extract_corr_data(
                data_array, time_level, level=level
            )
        else:
        ...
        if plot_type in ["line", "box", "xt", "tx"]:
            ...
            self.config_manager.ax_opts["central_lon"] = (
                self.config_manager.central_longitude
            )
            self.config_manager.ax_opts["central_lat"] = (
                self.config_manager.central_latitude
            )
            return (
                data2d[0],
                data2d[1],
        ...
        try:
            ...
            if not current_filename and hasattr(
                self.config_manager, "current_field_name"
            ):
            ...
            if is_regional:
                if hasattr(self, "_process_coordinates"):
                    self.logger.debug(
                        f"Calling self._process_coordinates for {field_name}"
                    )
                    return self._process_coordinates(
                        data2d,
                        dim1_name,
                        dim2_name,
                        field_name,
                    ...
                    if extent:
                        ...
                        self.config_manager.ax_opts["extent"] = extent
                        self.config_manager.ax_opts["central_lon"] = domain_info.get(
                            "central_lon", (lonW + lonE) / 2
                        )
                        self.config_manager.ax_opts["central_lat"] = domain_info.get(
                            "central_lat", (latS + latN) / 2
                        )
                        ...
                        if hasattr(data2d, "coords"):
                            ...
                            if (
                                lon_coord_name in data2d.coords
                                and lat_coord_name in data2d.coords
                            ):
                                ...
                                if len(lon_coords.dims) == 2:
                                    xs = np.array(
                                        lon_coords.isel({lon_coords.dims[0]: 0})
                                    )
                                    ys = np.array(
                                        lat_coords.isel({lat_coords.dims[1]: 0})
                                    )
                                else:
                        ...
                        self.logger.debug(
                            f"Extent: {self.config_manager.ax_opts['extent']} "
                        )
                        return (
                            data2d,
                            xs,
                        ...
                        self.logger.warning(
                            f"No domain extent available for regional data {field_name}"
                        )
    ...
    def _set_time_config(self, time_index, data_var):
        ...
        try:
            if "time" in data_var.coords:
                ...
                time_coords = [
                    coord for coord in data_var.coords if "time" in coord.lower()
                ]
                if time_coords:
    ...
    def process_simple_plots(self, plotter):
        ...
        for input_entry in self.config_manager.app_data.inputs:
            ...
            for field_name, plot_type_spec in to_plot.items():
                ...
                self.logger.info(
                    f"Processing simple plot: {field_name} ({', '.join(plot_types)})"
                )
                ...
                for i, data_source in enumerate(all_data_sources):
                    ...
                    if (
                        hasattr(data_source, "dataset")
                        and data_source.dataset is not None
                    ):
                    ...
                    if dataset is not None:
                        self.logger.debug(
                            f"Dataset has {len(dataset.data_vars)} data variables"
                        )
                        self.logger.debug(
                            f"Available variables: {list(dataset.data_vars.keys())[:10]}..."
                        )

                        if field_name in dataset.data_vars:
                            data = dataset[field_name]
                            self.logger.debug(
                                f"Found field '{field_name}' in data source {i}"
                            )
                            break
                        elif field_name in dataset.variables:
                            data = dataset[field_name]
                            self.logger.debug(
                                f"Found field '{field_name}' in variables of data source {i}"
                            )
                            break
                    else:
                ...
                if data is not None or dataset is not None:
                    ...
                    for plot_type in plot_types:
                        try:
                            self.logger.info(
                                f"Creating simple {plot_type} plot for {field_name}"
                            )
                            plotter.plot(
                                self.config_manager,
                                field_name,
                            ...
                            self.logger.error(
                                f"Failed to create simple {plot_type} plot for {field_name}: {str(e)}"
                            )
                            import traceback
                    ...
                    self.logger.warning(
                        f"Field '{field_name}' not found in any data source"
                    )
                    ...
                    for i, ds in enumerate(all_data_sources):
                        if hasattr(ds, "dataset") and ds.dataset is not None:
                            self.logger.debug(
                                f"  Data source {i} has {len(ds.dataset.data_vars)} variables"
                            )
                        else:
                            self.logger.debug(
                                f"  Data source {i} has no dataset or dataset is None"
                            )
    ...
    def process_single_plots(self):
        ...
        for idx, params in self.config_manager.map_params.items():
            ...
            self.config_manager.findex = self.config_manager.get_file_index_by_filename(
                filename
            )
    ...
    def process_comparison_plots(self):
        ...
        fields_file1 = [
            params["field"]
            for i, params in self.config_manager.map_params.items()
            if params["file_index"] == idx1
        ]
        fields_file2 = [
            params["field"]
            for i, params in self.config_manager.map_params.items()
            if params["file_index"] == idx2
        ]
        ...
        for field1, field2 in field_pairs:
            ...
            idx1_field = next(
                (
                    i
                    for i, params in self.config_manager.map_params.items()
                    if params["file_index"] == idx1 and params["field"] == field1
                ),
                None,
            )
            idx2_field = next(
                (
                    i
                    for i, params in self.config_manager.map_params.items()
                    if params["file_index"] == idx2 and params["field"] == field2
                ),
                None,
            )
            ...
            sdat1_dataset = (
                data_source1.dataset if hasattr(data_source1, "dataset") else None
            )
            sdat2_dataset = (
                data_source2.dataset if hasattr(data_source2, "dataset") else None
            )
            ...
            for plot_type in plot_types:
                ...
                if "xy" in plot_type or "po" in plot_type or "polar" in plot_type:
                    self._process_xy_comparison_plots(
                        file_indices,
                        current_field_index,
                        field1,
                        field2,
                    ...
                    self._process_other_comparison_plots(
                        file_indices,
                        current_field_index,
                        field1,
                        field2,
    ...
    def process_side_by_side_plots(self):
        ...
        for field1, field2 in field_pairs:
            ...
            for plot_type in plot_types:
                ...
                self.data2d_list = []
                if "xy" in plot_type or "polar" in plot_type:
                    self._process_xy_side_by_side_plots(
                        current_field_index,
                        field1,
                        field2,
                        plot_type,
                    ...
                    self._process_other_side_by_side_plots(
                        current_field_index,
                        field1,
                        field2,
                        plot_type,
    ...
    def process_corr_plots(self):
        ...
        if "ids" in corr_settings:
            ...
            if len(corr_ids) != 2:
                self.logger.error(
                    f"Expected exactly 2 experiment IDs for correlation, got {len(corr_ids)}: {corr_ids}"
                )
                return
            ...
            if len(file_indices) != 2:
                self.logger.error(
                    f"Could not find file indices for both experiment IDs: {corr_ids}"
                )
                return
            ...
            for idx in file_indices:
                fields = [
                    params.get("field")
                    for i, params in self.config_manager.map_params.items()
                    if params.get("file_index") == idx
                ]
                if fields:
                    field_names.append(
                        fields[0]
                    )  # Use the first field from each experiment
            ...
            for i, file_idx in enumerate(file_indices):
                ...
                params = next(
                    (
                        p
                        for _, p in self.config_manager.map_params.items()
                        if p.get("file_index") == file_idx
                        and p.get("field") == field_name
                    ),
                    None,
                )
                ...
                if params:
                    ...
                    if (
                        ...
                        for plot_type in plot_types:
                            if plot_type == "corr":
                                self.logger.info(
                                    f"Plotting {field_name}, {plot_type} plot"
                                )
                                # For correlation plots, we need to use the model object, not just the data source
                                self.logger.debug(
                                    f"Data source type: {type(data_source).__name__}"
                                )
                                self.logger.debug(
                                    f"Data source has process_plot: {hasattr(data_source, 'process_plot')}"
                                )
                                ...
                                if model_name:
                                    ...
                                    try:
                                        ...
                                        model_obj = model_factory.create(
                                            model_name, self.config_manager
                                        )
                                        if hasattr(model_obj, "process_plot"):
                                            self.logger.info(
                                                f"Using model object ({type(model_obj).__name__}) process_plot method"
                                            )
                                            model_obj.process_plot(
                                                field_data_array,
                                                field_name,
                                                file_idx,
                                                plot_type,
                                            ...
                                            self.logger.warning(
                                                f"Model object {type(model_obj).__name__} doesn't have process_plot method"
                                            )
                                            self.process_plot(
                                                field_data_array,
                                                field_name,
                                        ...
                                        self.logger.warning(
                                            f"Failed to create model object for {model_name}: {e}"
                                        )
                                        import traceback
                                    ...
                                    self.logger.info(
                                        f"No model name found, falling back to plot manager process_plot method"
                                    )
                                    self.process_plot(
                                        field_data_array,
                                        field_name,
            ...
            if len(corr_fields) != 2:
                self.logger.error(
                    f"Expected exactly 2 fields for correlation, got {len(corr_fields)}: {corr_fields}"
                )
                return
            ...
            for idx, params in self.config_manager.map_params.items():
                ...
                self.config_manager.findex = (
                    self.config_manager.get_file_index_by_filename(filename)
                )

                data_source = self.config_manager.pipeline.get_data_source(filename)
                if not data_source:
                    self.logger.warning(
                        f"No data source found in pipeline for {filename}"
                    )
                    continue
                        ...
                        if model_name:
                            ...
                            try:
                                ...
                                if hasattr(model_obj, "process_plot"):
                                    ...
                                    model_obj.process_plot(
                                        field_data_array, field_name, idx, plot_type
                                    )
                                else:
                                    self.logger.warning(
                                        f"Model object {type(model_obj).__name__} doesn't have process_plot method"
    ...
    def _process_xy_plot(
        ...
        zc_dim = (
            self.config_manager.get_model_dim_name_for_data("zc", data_array) or "lev"
        )
        ...
        if tave_setting and tc_dim in data_array.dims and num_times > 1:
            self.logger.info(
                f"Processing time-averaged plot for {field_name} with {num_times} time levels"
            )
        ...
        if self.config_manager.make_gif and len(time_levels) > 1:
            self.logger.info(
                f"Calculating global colorbar range for GIF consistency across {len(time_levels)} time frames"
            )
            global_vmin, global_vmax = self._calculate_global_minmax(
                data_array, field_name, time_levels, levels, tc_dim, zc_dim
            )
            if global_vmin is not None and global_vmax is not None:
                self.logger.info(
                    f"Using consistent colorbar range [{global_vmin:.2f}, {global_vmax:.2f}] for all {len(time_levels)} frames"
                )

        self._process_level_plot(
            data_array,
            field_name,
            file_index,
            plot_type,
    ...
    def _calculate_global_minmax(
        self, data_array, field_name, time_levels, levels, tc_dim, zc_dim
    ):
        ...
        for level_val in levels.keys():
            for t in time_levels:
                ...
                if not has_vertical_dim:
                    data2d = self.data_extractor._extract_xy_data(
                        data_at_time, 0, level=level_val
                    )
                else:
                    data2d = self.data_extractor._extract_xy_data(
                        data_at_time, 0, level=level_val
    ...
    def _process_level_plot(
        ...
        tc_dim = (
            self.config_manager.get_model_dim_name_for_data("tc", data_array) or "time"
        )
        ...
        for level_val in levels.keys():
            ...
            for t in time_levels:
                self.logger.debug(
                    f"Processing time level {t} for field {field_name}, level {level_val}"
                )
                if t == "average":
                    # Special case for time averaging - pass full time series
                    data_at_time = data_array
                    self.logger.info(
                        f"Using full time series for averaging, shape: {data_at_time.shape}"
                    )
                    ...
                    data_at_time = (
                        data_array.squeeze()
                    )  # Assume single time if no time dim

                if np.isnan(data_at_time).all():
                    self.logger.debug(
                        f"Skipping time level {t} for {field_name} - all values are NaN"
                    )
                    continue
                ...
                if not has_vertical_dim:
                    field_to_plot = self._prepare_field_to_plot(
                        data_at_time,
                        field_name,
                        file_index,
                        plot_type,
                ...
                if field_to_plot and not np.isnan(field_to_plot[0]).all():
                    ...
                    self.logger.warning(
                        f"Skipping plot for time level {t} - no valid data after processing"
                    )
    ...
    def _process_xt_plot(self, data_array, field_name, file_index, plot_type, figure):
        ...
        if field_to_plot:
            plot_result = self.create_plot(field_name, field_to_plot)
            pu.print_map(
                self.config_manager, plot_type, self.config_manager.findex, plot_result
            )
    ...
    def _process_polar_plot(
        self, data_array, field_name, file_index, plot_type, figure
    ):
    ...
    def _process_zsum_plot(
        self, data_array, field_name, file_index, plot_type, figure, time_levels
    ):
        ...
        for t in time_levels:
            ...
            field_to_plot = self._prepare_field_to_plot(
                data_at_time, field_name, file_index, plot_type, figure, time_level=t
            )
            if field_to_plot:
                plot_result = self.create_plot(field_name, field_to_plot)
                pu.print_map(
    ...
    def _process_scatter_plot(
        ...
        self.logger.debug(
            f"ax_opts projection: {self.config_manager.ax_opts.get('projection', 'NOT SET')}"
        )
        self.logger.debug(
            f"ax_opts extent: {self.config_manager.ax_opts.get('extent', 'NOT SET')}"
        )
        self.config_manager.level = None
        time_level_config = self.config_manager.ax_opts.get("time_lev", 0)
        tc_dim = self.config_manager.get_model_dim_name("tc") or "time"
        ...
        for t in time_levels:
            ...
            if field_to_plot:
                ...
                self.logger.warning(
                    f"No valid field_to_plot returned for time level {t}"
                )
    ...
    def _process_box_plot(self, data_array, field_name, file_index, plot_type, figure):
        ...
        if (
            hasattr(self.config_manager, "spec_data")
            and field_name in self.config_manager.spec_data
            and "boxplot" in self.config_manager.spec_data[field_name]
            and "time_lev" in self.config_manager.spec_data[field_name]["boxplot"]
        ):

            time_level_config = self.config_manager.spec_data[field_name]["boxplot"][
                "time_lev"
            ]

        # If not found in field-specific config, check ax_opts
        if time_level_config is None:
            time_level_config = self.config_manager.ax_opts.get(
                "time_lev", -1
            )  # Default to last time level

        if (
            isinstance(time_level_config, str)
            and time_level_config.strip("-").isdigit()
        ):
        ...
        if tc_dim in data_array.dims:
            ...
            if isinstance(time_level_config, int):
                actual_time_lev = (
                    time_level_config
                    if time_level_config >= 0
                    else num_times + time_level_config
                )

        exp_id = self.config_manager.get_file_exp_id(self.config_manager.findex)
        box_data = self.data_extractor._extract_box_data(
            data_array, time_lev=time_level_config, exp_id=exp_id
        )
        ...
        if isinstance(plot_result, tuple) and len(plot_result) >= 1:
            ...
            pu.print_map(
                self.config_manager, plot_type, self.config_manager.findex, fig
            )  # Pass just the figure
        else:
    ...
    def _process_csv_plot(self, data_array, field_name, file_index, plot_type, figure):
        ...
        self.logger.info(
            f"Processing CSV/categorical {plot_type} plot for {field_name}"
        )
        ...
        data_type = self.config_manager.map_params[file_index].get(
            "data_type", "gridded"
        )
        ...
        if data_type == "categorical":
            ...
            if (
                self.config_manager.spec_data
                and plot_type in self.config_manager.spec_data
                and plot_spec_key in self.config_manager.spec_data[plot_type]
            ):
                plot_opts = self.config_manager.spec_data[plot_type][plot_spec_key]
                self.logger.debug(
                    f"Using categorical spec options for {plot_type}.{plot_spec_key}: {plot_opts}"
                )
        else:
            ...
            if (
                self.config_manager.spec_data
                and field_name in self.config_manager.spec_data
                and plot_spec_key in self.config_manager.spec_data[field_name]
            ):
                plot_opts = self.config_manager.spec_data[field_name][plot_spec_key]
                self.logger.debug(
                    f"Using CSV spec options for {field_name}.{plot_spec_key}: {plot_opts}"
                )
    ...
    def _process_box_plots(
        ...
        if (
            hasattr(self.config_manager, "spec_data")
            and field_name1 in self.config_manager.spec_data
            and "boxplot" in self.config_manager.spec_data[field_name1]
            and "time_lev" in self.config_manager.spec_data[field_name1]["boxplot"]
        ):

            time_level_config = self.config_manager.spec_data[field_name1]["boxplot"][
                "time_lev"
            ]
        ...
        df1 = self.data_extractor._extract_box_data(
            sdat1_dataset[field_name1], time_level_config, exp_id1
        )
        ...
        for i, file_idx in enumerate(self.config_manager.b_list, start=1):
            ...
            df2 = self.data_extractor._extract_box_data(
                dataset[field_name2], time_level_config, exp_id2
            )
        ...
        self.logger.debug(
            f"Combined DataFrame has experiment column: {'experiment' in combined_df.columns}"
        )

        figure = Figure.create_eviz_figure(
            self.config_manager, plot_type, nrows=1, ncols=1
        )
        figure.set_axes()
        ...
        pu.print_map(
            self.config_manager, plot_type, self.config_manager.findex, self.plot_result
        )
    ...
    def _process_xy_side_by_side_plots(
        ...
        for level_val in levels:
            ...
            if not hasattr(self.config_manager, "colorbar_width"):
                self.config_manager.colorbar_width = (
                    0.02  # Consistent width for all colorbars
                )

            figure = Figure.create_eviz_figure(
                self.config_manager, plot_type, nrows=nrows, ncols=ncols
            )

            # Set up axes with proper spacing
            figure.set_axes()
            self.logger.info(
                f"Figure created with {nrows}x{ncols} layout for {num_plots} datasets"
            )
            self.config_manager.level = level_val
            ...
            if self.config_manager.is_regional:
                ...
                if (
                    lon_coord_name
                    and lat_coord_name
                    and lon_coord_name in sdat1_dataset.coords
                    and lat_coord_name in sdat1_dataset.coords
                ):
                    self.lon = sdat1_dataset.coords[lon_coord_name]
                    self.lat = sdat1_dataset.coords[lat_coord_name]
                    self.logger.debug(
                        f"Set coordinates: lon={lon_coord_name}, lat={lat_coord_name}"
                    )
                else:
                    self.logger.debug(
                        f"Coordinate names not found: lon={lon_coord_name}, lat={lat_coord_name}"
                    )
                    self.logger.debug(
                        f"Available coordinates: {list(sdat1_dataset.coords.keys())}"
                    )

            # Create the side-by-side plots
            self._create_xy_side_by_side_plot(
                current_field_index,
                field_name1,
                field_name2,
                figure,
            ...
            if hasattr(self.config_manager, "_filled_contours"):
                self.logger.debug(
                    f"Clearing filled contours list after processing {field_name1}"
                )
    ...
    def _process_other_side_by_side_plots(
        ...
        self.logger.info(
            f"Processing side-by-side comparison of {field_name1} vs {field_name2} for {plot_type} plot type"
        )

        # For most non-XY plot types, we'll use a generic approach that delegates to the appropriate plotter
        self._process_generic_side_by_side_plots(
            current_field_index,
            field_name1,
            field_name2,
            plot_type,
    ...
    def _process_generic_side_by_side_plots(
        self,
        current_field_index,
        field_name1,
        field_name2,
        ...
        self.logger.info(
            f"Processing generic side-by-side comparison of {field_name1} vs {field_name2} as {plot_type} data"
        )

        # Check if this is overlay mode
        is_overlay = (
            hasattr(self.config_manager, "overlay") and self.config_manager.overlay
        )
        ...
        if is_overlay:
            ...
            all_data_sources = self.config_manager.pipeline.get_all_data_sources()
            self.logger.info(
                f"Found {len(all_data_sources)} data sources from pipeline"
            )
                ...
                if (
                    ...
                    if field_name1 in dataset:
                        ...
                        if not any(id(ds) == id(dataset) for ds in datasets):
                            ...
                            self.logger.info(
                                f"Added dataset {i} from {file_path} to overlay list"
                            )
                        else:
                            self.logger.debug(
                                f"Dataset {i} from {file_path} already in list (duplicate)"
                            )
                    else:
                        self.logger.debug(
                            f"Field {field_name1} not found in dataset from {file_path}"
                        )
                else:
        ...
        self.logger.debug(
            f"Processing {len(datasets)} datasets for {plot_type} comparison"
        )

        if is_overlay:
            # For overlay mode, create a single figure and plot all datasets on it
            self._process_overlay_plots(
                datasets, field_name1, plot_type, current_field_index
            )
        else:
            # For side-by-side mode, create separate plots for each dataset
            self._process_separate_plots(datasets, field_name1, plot_type)

    def _process_overlay_plots(
        self, datasets, field_name, plot_type, current_field_index
    ):
        ...
        if plot_type == "xt":  # Only for time series plots
            ...
            for dataset in datasets:
                if field_name in dataset:
                    ...
                    if field_data.ndim >= 1:
                        ...
                        all_data_values.extend(
                            data_values[~np.isnan(data_values)]
                        )  # Remove NaN values
            ...
            if all_data_values:
                ...
                self.logger.debug(
                    f"Calculated global y-axis range for overlay: [{global_min:.6f}, {global_max:.6f}]"
                )
        ...
        for i, dataset in enumerate(datasets):
            ...
            field_data = dataset[field_name]
            self.logger.debug(
                f"Adding dataset {i} to overlay plot, data shape: {field_data.shape}"
            )
    ...
    def _process_separate_plots(self, datasets, field_name, plot_type):
        ...
        for i, dataset in enumerate(datasets):
            ...
            field_data = dataset[field_name]
            self.logger.debug(
                f"Processing field {field_name} from dataset {i}, data shape: {field_data.shape}"
            )
            ...
            if hasattr(self, "_process_xt_plot") and "xt" in plot_type:
                ...
                try:
                    ...
                    self.logger.warning(
                        "YZ plot method not available, using general approach"
                    )
                ...
                self.logger.info(
                    f"Using general process_plot for {plot_type} plot type"
                )
                self.process_plot(field_data, field_name, i, plot_type)

    def _create_xy_side_by_side_plot(
        self,
        current_field_index,
        field_name1,
        field_name2,
        ...
        if self.config_manager.a_list:
            ...
            self.logger.info(
                f"Processing first dataset {first_file_idx} with field {field_name1}"
            )
            self.logger.info(
                f"First dataset data shape: {first_data.shape}, has data: {not first_data.isnull().all().values}"
            )
            self._process_single_side_by_side_plot(
                first_file_idx,
                current_field_index,
                field_name1,
                figure,
        ...
        for i, file_idx in enumerate(self.config_manager.b_list, start=1):
            if i < num_plots:  # Only plot if we have a corresponding axis
                ...
                if field_name_for_dataset not in dataset.data_vars:
                    self.logger.warning(
                        f"Field '{field_name_for_dataset}' not found in dataset {filename}, skipping this plot"
                    )
                    continue

                self.logger.info(
                    f"Processing dataset {file_idx}: {filename} with field {field_name_for_dataset}"
                )
                field_data = dataset[field_name_for_dataset]
                self.logger.info(
                    f"Field data shape: {field_data.shape}, has data: {not field_data.isnull().all().values}"
                )
    ...
    def _process_single_side_by_side_plot(
        self,
        file_index,
        current_field_index,
        field_name,
        ...
        if self.config_manager.should_overlay_plots(field_name, plot_type[:2]):
            if file_index in self.config_manager.a_list:
                ...
                dataset_index = len(
                    self.config_manager.a_list
                ) + self.config_manager.b_list.index(file_index)
            else:
                dataset_index = 0

            self.config_manager.current_dataset_index = dataset_index
            self.config_manager.total_datasets = len(self.config_manager.a_list) + len(
                self.config_manager.b_list
            )
        ...
        self.logger.info(
            f"field_to_plot result for ax_index {ax_index}: {field_to_plot is not None}"
        )
        if field_to_plot and field_to_plot[0] is not None:
            data2d = field_to_plot[0]
            self.logger.info(
                f"Data stats for ax_index {ax_index}: min={float(data2d.min()):.6f}, max={float(data2d.max()):.6f}, shape={data2d.shape}"
            )
        ...
        if field_to_plot:
            self.logger.info(
                f"Creating plot for ax_index {ax_index}, field {field_name}"
            )
            self.plot_result = self.create_plot(field_name, field_to_plot)
            self.logger.info(
                f"Plot result for ax_index {ax_index}: {self.plot_result is not None}"
            )
    ...
    def _process_xy_comparison_plots(
        ...
        for level_val in levels:
            ...
            if figure.subplots == (3, 1):
                self._create_3x1_comparison_plot(
                    file_indices,
                    current_field_index,
                    field_name1,
                    field_name2,
                ...
                self._create_2x2_comparison_plot(
                    file_indices,
                    current_field_index,
                    field_name1,
                    field_name2,
    ...
    def _create_3x1_comparison_plot(
        self,
        file_indices,
        current_field_index,
        field_name1,
        ...
        self._process_3x1_comparison_plot(
            file_index1,
            current_field_index,
            field_name1,
            figure,
    ...
    def _create_2x2_comparison_plot(
        self,
        file_indices,
        current_field_index,
        field_name1,
        ...
        self._process_2x2_comparison_plot(
            file_index1,
            current_field_index,
            field_name1,
            figure,
    ...
    def _process_3x1_comparison_plot(
        self,
        file_index,
        current_field_index,
        field_name,
        ...
        if ax_index == 2:
            ...
            if len(self.data2d_list) == 2:
                ...
                proc = (
                    self.data_extractor.processor
                    if hasattr(self.data_extractor, "processor")
                    else None
                )
                if proc is None:
                ...
                self.logger.debug(
                    f"data2d1 shape: {data2d1.shape}, dims: {data2d1.dims}"
                )
                self.logger.debug(
                    f"data2d2 shape: {data2d2.shape}, dims: {data2d2.dims}"
                )

                # Regrid data2d2 to match data2d1's grid
                try:
                    # Regrid data2d2 to match data2d1's grid
                    d2_on_d1 = proc.regrid(
                        data2d1, data2d2, dims=(dim1_name, dim2_name)
                    )
    ...
    def _process_2x2_comparison_plot(
        self,
        file_index,
        current_field_index,
        field_name,
        ...
        if isinstance(data_array, tuple):
            ...
            if len(self.data2d_list) == 2:
                ...
                try:
                    ...
                    if self.config_manager.ax_opts.get("add_extra_field_type", False):
                        diff_result = proc.compute_difference(
                            data2d1,
                            d2_on_d1,
