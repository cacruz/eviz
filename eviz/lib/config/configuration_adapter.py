...
class ConfigurationAdapter:
    ...
    def process_configuration(self):
        ...
        for i, file_entry in enumerate(self.config_manager.app_data.inputs):
            ...
            try:
                source_name = self.config_manager.source_names[
                    self.config_manager.ds_index
                ]
            ...
            self.logger.debug(
                f"Loading data for file {i+1}: {file_path} with source_name: {source_name}"
            )
            ...
            try:
                try:
                    ...
                    if "got an unexpected keyword argument 'file_format'" in str(e):
                        data_source = self.config_manager._pipeline.process_file(
                            file_path, model_name=source_name, metadata=exp_metadata
                        )
                    else:
                        raise
        ...
        if (
            hasattr(self.config_manager.input_config, "_integrate")
            and self.config_manager.input_config._integrate
        ):
            self.logger.debug("Integrating datasets")
            try:
                integrated_dataset = (
                    self.config_manager._pipeline.integrate_data_sources()
                )
        ...
        if (
            hasattr(self.config_manager.input_config, "_composite")
            and self.config_manager.input_config._composite
        ):
            ...
            try:
                ...
                self.config_manager._pipeline.integrate_variables(
                    variables, operation, output_name
                )
