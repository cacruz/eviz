...
class DataExtractor:
    ...
    def _get_dimension_size(self, data_array, dim_name):
        ...
        # Then check if it's a coordinate
            ...
            self.logger.debug(
                f"Dimension '{dim_name}' not found in data array. Available dims: {list(data_array.dims)}, Available coords: {list(data_array.coords.keys())}"
            )
            return 0
    ...
    def _extract_scatter_data(self, data_array, time_level=None):
        ...
        """
        self.logger.debug(
            f"Starting scatter data extraction for time_level: {time_level}"
        )
        ...
        if tc_dim in d_temp.dims:
            ...
            if time_level == "all":
                self.logger.debug(
                    "'all' time levels requested, processing one at a time"
                )
                time_level = 0
        ...
        try:
            ...
            if xc_dim in d2d.coords and yc_dim in d2d.coords:
                ...
                if x.ndim == 2 and y.ndim == 2:
                    ...
                    if len(x) == len(data_flat) and len(y) == len(data_flat):
                        ...
                        self.logger.debug(
                            "Processing 1D coordinates (regular grid - creating meshgrid)"
                        )
            ...
            self.logger.debug(
                f"Array lengths before masking - x: {len(x_flat)}, y: {len(y_flat)}, values: {len(values)}"
            )

            # Verify all arrays have the same length before masking
            if not (len(x_flat) == len(y_flat) == len(values)):
                self.logger.error(
                    f"Dimension mismatch: x({len(x_flat)}), y({len(y_flat)}), values({len(values)})"
                )
    ...
    def _extract_xy_data(self, data_array, time_level, level=None):
        ...
        if num_tc == 0:
            ...
            self.logger.debug(
                f"Time dimension '{tc_dim}' not found in data array - assuming data is already time-sliced"
            )
        ...
        if self.config_manager.ax_opts.get(
            "zsum", False
        ) or self.config_manager.ax_opts.get("zave", False):
            if (
                zc_dim and zc_dim in dims_to_squeeze
            ):  # Check if zc_dim exists and is a singleton
                dims_to_squeeze.remove(
                    zc_dim
                )  # Do not squeeze zc_dim if zsum/zave is active
            ...
            if tc_dim is not None and tc_dim in data2d.dims:
                ...
                if num_tc > 1:
                    ...
                    return apply_conversion(
                        self.config_manager, data2d, data_array.name
                    )
            else:
                self.logger.debug(
                    f"Time averaging requested but no time dimension found (tc_dim={tc_dim}). Skipping time averaging."
                )
        ...
        if np.issubdtype(data2d.values.dtype, np.number):
            ...
            self.logger.debug(
                f"Skipping NaN check for non-numeric data type: {data2d.values.dtype}"
            )
    ...
    def _extract_xt_data(self, data_array, time_lev):
        ...
        if (
            ...
            and data_array.name in self.config_manager.spec_data
        ):
                ...
                if mean_type == "point_sel":
                    ...
                    try:
                        ...
                        if xc_dim in data2d.coords and yc_dim in data2d.coords:
                            data2d = data2d.sel(
                                {xc_dim: xc, yc_dim: yc}, method="nearest"
                            )
                        else:
                    ...
                    try:
                        ...
                        if xc_dim in data2d.coords and yc_dim in data2d.coords:
                            ...
                            if "lon" in data2d.coords and "lat" in data2d.coords:
                                data2d = data2d.sel(
                                    lon=slice(x1, x2), lat=slice(y1, y2)
                                )
                    ...
                    try:
                        ...
                        if tc_dim in data2d.dims:
                            data2d = data2d.rolling(
                                {tc_dim: window_size}, center=True
                            ).mean()
                        else:
                            if "time" in data2d.dims:
                                data2d = data2d.rolling(
                    ...
                    try:
                        ...
                        if tc_dim in data2d.dims:
                            non_time_dims = [
                                dim for dim in data2d.dims if dim != tc_dim
                            ]
                            if non_time_dims:
                            ...
                            if "time" in data2d.dims:
                                non_time_dims = [
                                    dim for dim in data2d.dims if dim != "time"
                                ]
                                if non_time_dims:
                                    data2d = data2d.mean(dim=non_time_dims)
                            else:
    ...
    def _extract_box_data(self, data_array, time_lev=None, exp_id=None):
        ...
        if tc_dim and tc_dim in d_temp.dims:
            ...
            if time_lev == "all" and tc_dim in d_temp.dims:
                ...
                if all_data:
                    ...
                    self.logger.debug(
                        f"for {valid_time_count} valid time levels (out of {num_times} total)"
                    )
                    return df
                else:
                    return None
        ...
        if np.issubdtype(d_temp.values.dtype, np.number):
            ...
            if np.isnan(d_temp.values).any():
                self.logger.debug(
                    f"Output contains NaN values: {np.sum(np.isnan(d_temp.values))} NaNs"
                )
        else:
            self.logger.debug(
                f"Skipping NaN check for non-numeric data type: {d_temp.values.dtype}"
            )
        ...
        try:
            ...
            if len(d_temp.dims) > 1:
                ...
                if tc_dim in d_temp.dims and (
                    time_lev == "all" or time_lev is None and len(d_temp[tc_dim]) > 1
                ):
                    time_values = d_temp[tc_dim].values
                    num_times = len(time_values)
                    ...
                    for t in range(num_times):
                        ...
                        if np.isnan(flat_data).all():
                            self.logger.debug(
                                f"Skipping time {time_str} - all values are NaN"
                            )
                            continue
                    ...
                    if all_data:
                        ...
                        self.logger.warning(
                            f"No valid data for box plot of {field_name}"
                        )
                        return None
                ...
                if np.isnan(flat_data).all():
                    self.logger.warning(
                        f"All values are NaN for {data_array.name if hasattr(data_array, 'name') else 'unnamed field'}"
                    )
                    return None
            ...
            if (
                ...
                if "boxplot" in self.config_manager.spec_data[field_name]:
                    if (
                        "fill_value"
                        in self.config_manager.spec_data[field_name]["boxplot"]
                    ):
                        fill_value = self.config_manager.spec_data[field_name][
                            "boxplot"
                        ]["fill_value"]
            ...
            if len(df) > max_points:
                self.logger.debug(
                    f"Sampling {max_points} points from {len(df)} total points"
                )
    ...
    def _extract_line_data(self, data_array, time_lev=None, level=None):
        ...
        try:
            ...
            if tc_dim in data_array.dims:
                ...
                spatial_dims = [
                    dim for dim in [xc_dim, yc_dim, zc_dim] if dim in data_array.dims
                ]
                ...
                if spatial_dims:
                    ...
                    if (
                        x_point is not None
                        and y_point is not None
                        and xc_dim in data_array.dims
                        and yc_dim in data_array.dims
                    ):
                        data_array = data_array.sel(
                            {xc_dim: x_point, yc_dim: y_point}, method="nearest"
                        )
            ...
            if (
                ...
                if "fill_value" in self.config_manager.spec_data[field_name].get(
                    "lineplot", {}
                ):
                    fill_value = self.config_manager.spec_data[field_name]["lineplot"][
                        "fill_value"
                    ]
            ...
            if "txplot" in spec:
                ...
                if "yrange" in spec["txplot"]:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error applying latitude range selection: {e}"
                        )
