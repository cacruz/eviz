...
class DataIntegrator:
    ...
    def integrate_data_sources(
        self, data_sources: List[DataSource], **kwargs
    ) -> xr.Dataset:
        ...
        if method == "merge":
            ...
            return self._concatenate_datasets(
                [ds.dataset for ds in data_sources], **kwargs
            )
        else:
    ...
    def _concatenate_datasets(self, datasets: List[xr.Dataset], **kwargs) -> xr.Dataset:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully concatenated {len(datasets)} datasets along dimension '{dim}'"
            )
            return result
        except Exception as e:
    ...
    def integrate_variables(
        ...
        try:
            ...
            dataset[output_name].attrs[
                "long_name"
            ] = f"{operation.capitalize()} of {', '.join(variables)}"
            dataset[output_name].attrs["operation"] = operation
            dataset[output_name].attrs["source_variables"] = variables

            self.logger.info(
                f"Successfully integrated variables {variables} with operation '{operation}'"
            )
            return dataset
    ...
    def integrate_datasets_by_time(
        self, datasets: List[xr.Dataset], **kwargs
    ) -> xr.Dataset:
        ...
        for i, ds in enumerate(datasets):
            if time_dim not in ds.dims:
                self.logger.warning(
                    f"Time dimension '{time_dim}' not found in dataset {i}"
                )
                return datasets[0]  # Return the first dataset as a fallback
        ...
        try:
            ...
            self.logger.info(
                f"Successfully integrated {len(datasets)} datasets by time"
            )
            return result
        except Exception as e:
