...
class GriddedDataSource(GenericDataSource):
    ...
    def process_data(self, dataset: xr.Dataset) -> dict:
        ...
        """
        self.logger.debug(
            f"Processing gridded dataset with variables: {list(dataset.data_vars)}"
        )

        # For gridded data, we typically don't need to transform the dataset structure
        # The processing is handled by the DataExtractor and PlotManager
        processed_data = {
            "dataset": dataset,
            "type": "gridded",
    ...
    def validate_data(self, dataset: xr.Dataset) -> bool:
        ...
        # Check for required coordinate dimensions (at least lat/lon or x/y)
        coords = set(dataset.coords.keys())
        dims = set(dataset.dims.keys())

        # Look for spatial coordinates
        has_spatial = any(
            [
                # Standard lat/lon coordinates
                ("lat" in coords or "latitude" in coords)
                and ("lon" in coords or "longitude" in coords),
                # Alternative x/y coordinates
                ("x" in coords and "y" in coords)
                or ("xc" in coords and "yc" in coords),
                # Grid coordinates
                ("i" in dims and "j" in dims),
            ]
        )

        if not has_spatial:
            self.logger.warning("Dataset does not appear to have spatial coordinates")
            return True  # Allow processing anyway, might be 1D data
        ...
        self.logger.debug("Dataset validation passed")
        return True
    ...
    def process_simple_plots(self, plotter):
        ...
        for i in map_params.keys():
            ...
            for plot_type in map_params[i]["to_plot"]:
                ...
                field_to_plot = self._get_field_for_simple_plot(
                    field_data_array, field_name, plot_type
                )
                plotter.simple_plot(self.config_manager, field_to_plot)
            field_num += 1

    # Simple plots do not use configurations in SPECS file
    def _get_field_for_simple_plot(
        self, data_array: xr.DataArray, field_name: str, plot_type: str
    ) -> tuple:
        ...
        dim1_coords = (
            data2d[dim1_name] if dim1_name and dim1_name in data2d.coords else None
        )
        dim2_coords = (
            data2d[dim2_name] if dim2_name and dim2_name in data2d.coords else None
        )
            ...
            for t in time_levels:
                ...
                if not has_vertical_dim:
                    field_to_plot = self._prepare_field_to_plot(
                        data_at_time, field_name, file_index, plot_type, figure, t
                    )
                else:
                    field_to_plot = self._prepare_field_to_plot(
                        data_at_time,
    ...
    def _process_other_side_by_side_plots(
        ...
        use_overlay = self.config_manager.should_overlay_plots(
            field_name1, plot_type[:2]
        )
        if use_overlay:
