...
class ObservationalDataSource(GenericDataSource):
    ...
    def process_data(self, dataset: xr.Dataset) -> dict:
        ...
        """
        self.logger.debug(
            f"Processing observational dataset with variables: {list(dataset.data_vars)}"
        )

        # For observational data, we may need special handling for swath data,
    ...
    def validate_data(self, dataset: xr.Dataset) -> bool:
        ...
        # Observational data is more flexible than gridded data
        # It may have irregular coordinates, swath data, etc.
        coords = set(dataset.coords.keys())

        # Look for any kind of coordinate information
        has_coords = len(coords) > 0

        if not has_coords:
            self.logger.warning("Dataset has no coordinate information")
            # Still allow processing for simple observational data

        self.logger.debug("Observational dataset validation passed")
        return True
    ...
    def get_data_extent(self, data_array: xr.DataArray) -> list:
        ...
        try:
            ...
            if all(
                var in locals() for var in ["lon_min", "lon_max", "lat_min", "lat_max"]
            ):
                lon_buffer = (lon_max - lon_min) * 0.05
                lat_buffer = (lat_max - lat_min) * 0.05
                extent = [
    ...
    def _extract_xy_data(self, data_array, time_level=None, level=None):
        ...
        if data2d is not None:
            ...
            if "extent" not in self.config_manager.ax_opts:
                self.apply_extent_to_config(
                    data2d, data_array.name if hasattr(data_array, "name") else None
                )
    ...
    def _process_corr_plot(self, data_array, field_name, file_index, plot_type, figure):
        ...
        if "ids" in corr_settings:
            ...
            if reference_exp_id:
                ...
                if reference_file_index is not None:
                    ...
                    map_params = self.config_manager.map_params.get(
            ...
            if hasattr(self.config_manager, "pipeline"):
                ...
                for source_name, data_source in all_data_sources.items():
                    if (
                        ...
                        and reference_field in data_source.dataset
                    ):
                ...
                if reference_data is None:
                    try:
                        reference_data = self.config_manager.pipeline.get_variable(
                            reference_field
                        )
                    except (AttributeError, KeyError) as e:
                        self.logger.warning(
                            f"Could not get reference field using get_variable: {e}"
                        )
                ...
                if reference_data is None:
                    try:
                        ...
                        self.logger.warning(
                            f"Could not get reference field from all variables: {e}"
                        )
        ...
        reference_to_plot = self._prepare_field_to_plot(
            reference_data,
            reference_field,
            file_index,
            plot_type,
