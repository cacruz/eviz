    ...
    def process_plot(self, data_array, field_name, file_index, plot_type):
        ...
        if plot_type == 'sc':
            ...
            if hasattr(handler, '_process_corr_plot'):
                handler._process_corr_plot(data_array, field_name, file_index, plot_type, figure)
            else:
                self.logger.warning(f"_process_corr_plot not implemented for {handler.__class__.__name__}")
