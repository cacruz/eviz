...
class Grib(GriddedSource):
    ...
    def _process_xy_plot(
        ...
        do_zsum = self.config_manager.ax_opts.get(
            "zsum", False
        ...
        if levels:
            ...
            self._process_zsum_plots(
                data_array, field_name, file_index, plot_type, figure, time_levels
            )

    def _process_level_plots(
        self,
        data_array: xr.DataArray,
        field_name: str,
        file_index: int,
    ...
    def _get_data(self, field_name, ax_opts, pid):
        d = self.config_manager.readers[0].get_field(
            field_name, self.config_manager.findex
        )
    ...
    def _process_coordinates(
        self, data2d, dim1, dim2, field_name, plot_type, file_index, figure, ax
    ):
        """
    ...
    def _apply_time_selection(
        self, original_data, data2d, time_dim, time_lev, field_name, level
    ):
        """
        ...
        if time_dim and time_dim in original_data.dims:
            ...
            if (
                hasattr(self, "ax_opts")
                and self.ax_opts.get("tave", False)
                and num_times > 1
            ):
