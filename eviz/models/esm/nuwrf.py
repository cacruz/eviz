...
class NuWrf(GriddedSource):
    ...
    def process_simple_plots(self, plotter):
        ...
        for i in map_params.keys():
            ...
            self._global_attrs = self.set_global_attrs(
                source_name, self.source_data["attrs"]
            )
    ...
    def _get_field_for_simple_plot(self, field_name, plot_type):
        ...
        raise NotImplementedError(
            "Subclasses must implement _get_field_for_simple_plot"
        )
        ...
        if zname in data2d.dims:
            if "soil" in zname:
                ...
                if self.source_name == "lis":
                    ...
                    lev_to_plot = self.levs[
                        index
                    ]  # should I use this instead of index?
        ...
        if source_name == "wrf":
            ...
            if zc:
                for name in self.get_model_dim_name("zc").split(","):
                    if (
                        hasattr(source_data, "coords")
                        and name in source_data.coords.keys()
                    ):
                        coords.append(name)
                        break
    ...
    def find_matching_dimension(self, field_dims, dim_name):
        ...
        for dim in field_dims:
            if (
                dim
                in self.config_manager.meta_coords[dim_name][self.source_name]["dim"]
            ):
                return dim
        return None
