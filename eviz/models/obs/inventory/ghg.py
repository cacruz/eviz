...
class Ghg(ObsSource):
    ...
    def _get_field_for_simple_plot(
        self, field_name: str, plot_type: str
    ) -> Union[tuple, None]:
        """
        ...
        # Default to time series for GHG data
            ...
            self.logger.warning(
                f"Plot type {plot_type} may not be suitable for GHG data, defaulting to time series"
            )
            time_dim = list(data.coords.keys())[0]
            time_values = data.coords[time_dim].values
    ...
    def process_single_plots(self):
        ...
        for idx, params in self.config_manager.map_params.items():
            ...
            for plot_type in plot_types:
                self._process_plot(
                    field_data_array, field_name, idx, plot_type, processed_data
                )
    ...
    def _process_plot(
        self, data_array, field_name, file_index, plot_type, full_dataset=None
    ):
        """
    ...
    def process_comparison_plots(self, plotter):
        ...
        for field1, field2 in field_pairs:
            ...
            # Get the plot types
            plot_types = self.config_manager.map_params[idx1_field].get(
                "to_plot", ["xt"]
            )
            if isinstance(plot_types, str):
                plot_types = [pt.strip() for pt in plot_types.split(",")]
            ...
            for plot_type in plot_types:
                self.logger.info(
                    f"Plotting comparison of {field1} vs {field2}, {plot_type} plot"
                )

                # Create a figure for the comparison
                figure = Figure.create_eviz_figure(
                    self.config_manager, plot_type, nrows=2, ncols=1
                )
                figure.set_axes()
                ...
                field_to_plot1 = self._prepare_field_to_plot(
                    data1[field1],
                    field1,
                    idx1_field,
                    plot_type,
                ...
                field_to_plot2 = self._prepare_field_to_plot(
                    data2[field2],
                    field2,
                    idx2_field,
                    plot_type,
                ...
                # Print the map
                pu.print_map(
                    self.config_manager, plot_type, self.config_manager.findex, figure
                )
    ...
    def process_side_by_side_plots(self, plotter):
        ...
        for field1, field2 in field_pairs:
            ...
            for plot_type in plot_types:
                self.logger.info(
                    f"Plotting side-by-side {field1} vs {field2}, {plot_type} plot"
                )

                # Create a figure for the side-by-side comparison
                figure = Figure.create_eviz_figure(
                    self.config_manager, plot_type, nrows=1, ncols=2
                )
                figure.set_axes()
