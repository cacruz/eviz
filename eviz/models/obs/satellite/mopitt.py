...
class Mopitt(GenericSource):
    ...
    def prepare_data(self, fid) -> dict:
        ...
        for i in map_params.keys():
            ...
            da_new = da_new.where(da_new != -9999.0)
        ...
        unzipped_data["filename"] = "".join(
            self.config.file_list[self.config.findex]["filename"]
        )
                ...
                if source_name == self.config.source_names[s]:
                    ...
                    for pt in map_params[i]["to_plot"]:
                        ...
                        if "xy" in pt:
                            ...
                            if not levels:
                                self.logger.warning(
                                    f" -> No levels specified for {field_name}"
                                )
                                continue
                            for level in levels:
                            ...
                            field_to_plot = self._prepare_field_to_plot(
                                source_data, field_name, file_index, pt, figure
                            )
                            if self.use_mp_pool:
                                p = multiprocessing.Process(
                                    target=plotter.single_plots,
                                ...
                                plotter.single_plots(
                                    self.config, field_to_plot=field_to_plot
                                )
    ...
    def _prepare_field_to_plot(
        self, source_data, field_name, file_index, plot_type, figure, level=None
    ):
        ...
        data2d = None
        if "xy" in plot_type:
            data2d = self._extract_xy_data(
                source_data,
                field_name,
                level=level,
                time_lev=self.config.ax_opts["time_lev"],
            )
        else:
            pass
