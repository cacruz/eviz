...
class Omi(ObsSource):
    ...
    def process_single_plots(self):
        ...
        for idx, params in self.config_manager.map_params.items():
            ...
            #    ColumnAmountO3
            new_names = {
                name: name.split("/")[-1] for name in data_source.dataset.data_vars
            }
    ...
    def _process_plot(
        self, ds_short: xr.Dataset, field_name: str, file_index: int, plot_type: str
    ):
    ...
    def _process_obs_plot(
        ...
        field_to_plot = self._prepare_field_to_plot(
            ds_short, field_name, file_index, plot_type, figure, time_level=time_level
        )
        if field_to_plot:
...
def extract_field_with_coords(
    ds, field_name, lat_bounds=(-90, 90), lon_bounds=(-180, 180)
) -> tuple:
