...
class GenericSource(BaseSource):
    ...
    def __post_init__(self):
        ...
        self.data2d_list = (
            []
        )  # This might still be needed if other parts of the code rely on it, or can be removed if fully encapsulated by PlotOrchestrator

        if self.use_mp_pool:
            # Set to avoid establishing a GUI in each sub-process:
            matplotlib.use(
                "agg"
            )  # Keep this if matplotlib is still used directly for some reason, otherwise remove
            self.procs = list()
    ...
    def process_data(self, dataset: xr.Dataset) -> dict:
        ...
        """
        self.logger.debug(
            f"Processing dataset with variables: {list(dataset.data_vars) if dataset else 'None'}"
        )
            ...
            for field_name, plot_type in to_plot.items():
                ...
                if data is not None:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to create simple plot for {field_name}: {str(e)}"
                        )
                        import traceback
