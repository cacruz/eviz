...
class TestYAMLParser:
    ...
    @patch('eviz.lib.utils.read_meta_attrs')
    @patch('eviz.lib.utils.read_species_db')
    def test_parse_categorical_data_type(self, mock_species, mock_attrs, mock_coords, mock_load):
        """Test parsing categorical data type with new schema."""
        mock_load.return_value = {
            'inputs': [{
                'name': 'cars.csv',
                'location': '/data',
        ...
        # Should create 3 map_params entries (one for each plot type)
        assert len(parser.map_params) == 3

        # Check hist entry
        hist_entry = [e for e in parser.map_params.values() if e['field'] == 'hist'][0]
        assert hist_entry['data_type'] == 'categorical'
        assert hist_entry['to_plot'] == ['hist']
        assert hist_entry['plot_params'] == {'x': 'mpg', 'bins': 20}

        # Check bar entry
        bar_entry = [e for e in parser.map_params.values() if e['field'] == 'bar'][0]
        assert bar_entry['data_type'] == 'categorical'
        assert bar_entry['to_plot'] == ['bar']
        assert bar_entry['plot_params'] == {'x': 'origin', 'y': 'horsepower', 'agg': 'mean'}

        # Check scatter entry
        scatter_entry = [e for e in parser.map_params.values() if e['field'] == 'scatter'][0]
        assert scatter_entry['data_type'] == 'categorical'
        assert scatter_entry['to_plot'] == ['scatter']
        assert scatter_entry['plot_params'] == {'x': 'weight', 'y': 'mpg', 'color': 'origin'}

    @patch('eviz.lib.utils.load_yaml_simple')
    @patch('eviz.lib.utils.read_meta_coords')
    @patch('eviz.lib.utils.read_meta_attrs')
    @patch('eviz.lib.utils.read_species_db')
    def test_parse_gridded_data_type_default(self, mock_species, mock_attrs, mock_coords, mock_load):
        """Test that data_type defaults to 'gridded' for backward compatibility."""
        mock_load.return_value = {
            'inputs': [{
                'name': 'test.nc',
                'location': '/data',
        ...
        # Check that data_type defaults to 'gridded'
        map_entry = parser.map_params[0]
        assert map_entry['data_type'] == 'gridded'
        assert map_entry['field'] == 'temperature'
        assert map_entry['to_plot'] == ['xy', 'xt']

    @patch('eviz.lib.utils.load_yaml_simple')
    @patch('eviz.lib.utils.read_meta_coords')
    @patch('eviz.lib.utils.read_meta_attrs')
    @patch('eviz.lib.utils.read_species_db')
    def test_parse_gridded_with_explicit_data_type(self, mock_species, mock_attrs, mock_coords, mock_load):
        """Test parsing gridded data with explicit data_type."""
        mock_load.return_value = {
            'inputs': [{
                'name': 'air.nc',
                'location': '/data',
        ...
        # Check that gridded data is parsed correctly
        map_entry = parser.map_params[0]
        assert map_entry['data_type'] == 'gridded'
        assert map_entry['field'] == 'air'
        # Note: plot_types and options are not directly in to_plot for this format
        # The parser will extract them properly

    @patch('eviz.lib.utils.load_yaml_simple')
    @patch('eviz.lib.utils.read_meta_coords')
    @patch('eviz.lib.utils.read_meta_attrs')
    @patch('eviz.lib.utils.read_species_db')
    def test_categorical_plot_type_constants(self, mock_species, mock_attrs, mock_coords, mock_load):
        """Test that categorical plot type constants are defined."""
        assert 'bar' in YAMLParser.CATEGORICAL_PLOT_TYPES
        assert 'pie' in YAMLParser.CATEGORICAL_PLOT_TYPES
        assert 'hist' in YAMLParser.CATEGORICAL_PLOT_TYPES
        assert 'scatter' in YAMLParser.CATEGORICAL_PLOT_TYPES
        assert 'box' in YAMLParser.CATEGORICAL_PLOT_TYPES

    @patch('eviz.lib.utils.load_yaml_simple')
    @patch('eviz.lib.utils.read_meta_coords')
    @patch('eviz.lib.utils.read_meta_attrs')
    @patch('eviz.lib.utils.read_species_db')
    def test_data_type_constants(self, mock_species, mock_attrs, mock_coords, mock_load):
